...
class MediaHelper
{
    ...
    public static function insert_media_gallery_images($file, $type = 'admin')
    {
        ...
        // Resize and save the full image
        $resize_full_image = Image::make($file)->resize($image_width, $image_height, function ($constraint) {
        ...
        // Check if image width is greater than 150 and create additional sizes
        if ($image_width > 150) {
            ...
            foreach ([$thumb_image_folder, $grid_image_folder, $large_image_folder, $semi_large_image_folder, $tiny_image_folder] as $folder) {
                if (!is_dir($folder)) {
                    mkdir($folder, 0755, true);
                }
            }

            // Resize and save additional image sizes
            $resize_thumb_image = Image::make($file)->resize(150, 150);
            $resize_thumb_image->save($thumb_image_folder . 'thumb-' . $image_db);

            $resize_grid_image = Image::make($file)->resize(350, null, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_grid_image->save($grid_image_folder . 'grid-' . $image_db);

            $resize_large_image = Image::make($file)->resize(740, null, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_large_image->save($large_image_folder . 'large-' . $image_db);

            $resize_semi_large_image = Image::make($file)->resize(540, 350, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_semi_large_image->save($semi_large_image_folder . 'semi-large-' . $image_db);

            $resize_tiny_image = Image::make($file)->resize(15, 15)->blur(50);
            $resize_tiny_image->save($tiny_image_folder . 'tiny-' . $image_db);
        }

        return MediaUpload::latest('id')->first()->id ?? null;
    }
