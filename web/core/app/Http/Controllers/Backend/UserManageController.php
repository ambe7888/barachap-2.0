...
class UserManageController extends Controller
{
    ...
    public function all_users(Request $request)
    {
        ...
        $query = User::with('identity_verify')->latest();
        ...
        $total_client = User::where('type', 1)->count();
        $total_provider = User::where('type', 0)->count();

        return view('backend.pages.user.users.all-users',compact('all_users', 'current_status', 'total_client', 'total_provider', 'total_users'));
    }
    // user pagination
    ...
    public function permanent_delete($user_id)
    {
        try {
            ...
            $service_ids = Service::where('provider_id', $user_id)->pluck('id');
            Service::whereIn('id', $service_ids)->delete();

            $service = Service::where('provider_id', $user_id)->first();
            if (!empty($service)){
                $service_id = $service->id;
                ServiceAddon::where('service_id', $service_id)->delete();
                ServiceFaq::where('service_id', $service_id)->delete();
                ServiceExclude::where('service_id', $service_id)->delete();
                ServiceInclude::where('service_id', $service_id)->delete();
            }
            ...
            if (moduleExists("Chat")) {
                ...
                if (empty($live_chats)){
                    $live_chats = LiveChat::where('provider_id', $user_id)->get();
                }
            ...
            // if client or provider
            try {
                JobPost::where('client_id', $user_id)->delete();
                JobPostOffer::where('client_id', $user_id)->delete();
                JobPostLocation::where('job_post_id', $user_id)->delete();
                Staff::where('provider_id', $user_id)->delete();
                ...
                UserBalance::where('user_id', $user_id)->delete();
    ...
    public function user_restore(Request $request, $id=null)
    {
        if($request->isMethod('post')){
            ...
            $account_deactivate=AccountDeactivate::where('user_id', $id)->first();
            if ($account_deactivate){
                $account_deactivate->status = 0;
