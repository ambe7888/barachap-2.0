...
class ScheduleManageController extends Controller
{
    public function add_schedule(Request $request)
    {
        if($request->isMethod('post')){
            $request->validate([
                'day' => [new DayRequiredRule],
                'schedule_for_all_days' => 'nullable',
                'schedule' => ['required','regex:/^\d{1,2}\.\d{2}(AM|PM)-\d{1,2}\.\d{2}(AM|PM)$/i'],
            ],
            [
                'schedule.regex'=>__('Please enter time in format like 10.00AM-2.00PM or 10.00PM-2.00AM')
            ...
            if($request->has('schedule_for_all_days') && $request->schedule_for_all_days == 1){
                ...
                foreach($days as $day){
                    Schedule::create([
                        ...
                        'status' => 0,
                        'allow_multiple_schedule' => $request->schedule_for_all_days ?? 1,
                    ]);
                }
            }else{
                Schedule::create([
        ...
        return view('frontend.frontend.provider.pages.schedules.add-schedule');
    }
    //all user
    public function all_schedules()
    {
        $provider=Auth::user();
        $provider_id=$provider->id;
        $schedules = Schedule::whereNotNull('provider_id')
            ->where('provider_id', $provider_id)
            ->latest()
            ->paginate(10);

        return view('frontend.frontend.provider.pages.schedules.all-schedules',compact('schedules'));
    }
    ...
    function schedule_paginate(Request $request)
    {
        if($request->ajax()){
            ...
            $schedules = Schedule::whereNotNull('provider_id')
                ...
                ->latest()
                ->paginate(10);
            return view('frontend.frontend.provider.pages.schedules.search-result',compact('schedules'));
        }
    }
    ...
    public function search_schedule(Request $request)
    {
        ...
        return $schedules->total() >= 1 ? view('frontend.frontend.provider.pages.schedules.search-result', compact('schedules'))
            ->render() : response()
