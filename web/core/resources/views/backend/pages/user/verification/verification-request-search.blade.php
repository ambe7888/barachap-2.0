    ...
    @if($all_requests->total() >= 1)
        @foreach($all_requests as $req)
            <tr>
                ...
                <td>{{ optional($req->user)->fullname }}</td>
                <td>{{ optional($req->user)->email }}</td>
                <td>{{ optional($req->user)->phone }}</td>
                <td>
                    <x-status.table.active-inactive :status="optional($req->user)->status"/>
                </td>
                <td class="verified_status_load_{{$req->user_id}}">
                    <x-status.table.verified-status :status="optional($req->user)->verified_status"/>
                    @if($req->status === 2)
