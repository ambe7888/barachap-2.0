                ...
                <div class="userDetails__wrapper userProfile__details mt-3 ">
                    @if(!empty($user_identity_details))
                        <div class="userProfile__details__thumb mb-3 d-flex gap-3">
                            <a class="radius-5"
                               href="{{ asset('assets/uploads/verification/'.$user_identity_details->front_document) }}"
                               target="_blank">
                                @if(pathinfo($user_identity_details->front_document, PATHINFO_EXTENSION) === 'pdf')
                                    <a class="btn btn-info radius-5"
                                       href="{{ asset('assets/uploads/verification/'.$user_identity_details->front_document) }}"
                                       target="_blank">
                                ...
                                @else
                                    <div class="document-preview">
                                        <img src="{{ asset('assets/uploads/verification/'.$user_identity_details->front_document) }}"
                                             alt="front-document">
                                    </div>
                                @endif
                            </a>
                            <a class="radius-5"
                               href="{{ asset('assets/uploads/verification/'.$user_identity_details->back_document) }}"
                               target="_blank">
                                ...
                                @else
                                    <div class="document-preview">
                                        <img src="{{ asset('assets/uploads/verification/'.$user_identity_details->back_document) }}"
                                             alt="back-document">
                                    </div>
                                @endif
                            </a>
                        ...
                        @endphp
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('Verify by:') }}</strong> {{ $verify_by_name->name ?? '' }}</p>
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('Zip Code:') }}</strong> {{ $user_identity_details->zip_code ?? '' }}</p>
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('Address:') }}</strong> {{ $user_identity_details->address ?? '' }}</p>
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('Country:') }}</strong> {{ $user_identity_details->country }}
                        </p>
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('State:') }}</strong> {{ $user_identity_details->state }}
                        </p>
                        <p class="userDetails__wrapper__item">
                            <strong>{{ __('City:') }}</strong> {{ $user_identity_details->city }}
                        </p>
                    @else
