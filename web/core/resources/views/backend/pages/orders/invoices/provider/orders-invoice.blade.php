        ...
        <div class="invoice-header">
            <div class="invoice-flex-contents">
                ...
                <div class="invoice-header-contents">
                    <h2 class="invoice-title">{{ get_static_option('invoice_title') ?? __('INVOICE') }}</h2>
                </div>
            </div>
        </div>
        <div style="text-align: right; align-items: end; justify-content: end">
            <span><strong>{{  get_static_option('invoice_title') ?? __('Invoice No:') }}</strong> {{ $order_details->invoice_number }} </span>
        </div>
        <div class="invoice-details" style="margin-bottom:150px">

                 @include('backend.pages.orders.invoices.provider.order-invoice-info')

                    {{-- sub order invoice location start--}}
                    @if($order_details->subOrderLocations->isNotEmpty())
                    <div class="item-description" style="margin-top: 120px!important;">
                        <div class="table-responsive">
                            ...
                            <table class="custom--table">
                                ...
                                <tbody>
                                @if($order_details->subOrderLocations && $order_details->subOrderLocations->isNotEmpty())
                                    @foreach($order_details->subOrderLocations as $location)
                                        <tr>
                                            <td width="100px">
                     ...
                     @endif
                    {{-- sub order invoice location end--}}
                    {{-- sub order invoice addon start--}}
                       @if($order_details->subOrderAddons->isNotEmpty())
                        <div class="item-description" style="margin-top: 120px!important;">
                            <div class="table-responsive">
                                ...
                                <table class="custom--table">
                                    ...
                                    <tbody>
                                    @foreach($order_details->subOrderAddons as $subOderAddon)
                                        <tr>
                                            <td width="200px">
                                                <span class="data-span"> {{ $subOderAddon->id }}</span>
                                            </td>
                                            <td width="200px">
                                                <span class="data-span"> {{ $subOderAddon->title }}</span>
                                            </td>
                                            <td>
                                                <span class="data-span"> {{ float_amount_with_currency_symbol($subOderAddon->price) }}</span>
                                            </td>
                                            <td>
                                                <span class="data-span"> {{ $subOderAddon->quantity }}</span>
                                            </td>
                                            <td>
                                                {{ float_amount_with_currency_symbol($subOrder->total) }} <br>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                      ...
                      @endif
                    {{-- sub order invoice addon end--}}
            <div class="item-description">
                <h6>{{ __('Notes:') }}</h6>
                <span>{{ get_static_option('order_invoice_notes') }}</span>
            </div>
        </div>
        <footer>
