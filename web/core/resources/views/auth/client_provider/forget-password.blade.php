...
@section('content')
    <section class="loginForm d-flex">
        <div class="loginForm__left__inner desktop-center">
            <div class="loginForm__header">
                <h2 class="loginForm__header__title">{{ __('Forget Password') }}</h2>
                <p class="loginForm__header__para mt-3">{{ __('Hello there, here you can reset you password.') }} </p>
            </div>
            <x-validation.error/>
            <div class="loginForm__wrapper mt-4">
                <form action="{{ route('user.forget.password') }}" class="custom_form" method="POST">
                    @csrf
