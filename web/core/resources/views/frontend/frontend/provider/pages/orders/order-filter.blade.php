            ...
            <div class="dashboard_order__tab d-flex flex-wrap gap-3">
                <ul class="tabs">
                    <li class="{{ $current_status === 'all' ? 'active' : '' }}">
                        <a href="{{ route('provider.all.orders') }}">{{ __('All Orders') }} <span class="badge_notification">{{ $total_orders }}</span></a>
                    </li>
                    <li class="{{ $current_status === '0' ? 'active' : '' }}">
                        <a href="{{ route('provider.all.orders', ['status' => '0']) }}">{{ __('Pending') }} <span class="badge_notification">{{ $total_pending_order }}</span></a>
                    </li>
                    <li class="{{ $current_status === '1' ? 'active' : '' }}">
                        <a href="{{ route('provider.all.orders', ['status' => '1']) }}">{{ __('Completed') }} <span class="badge_notification">{{ $total_completed_order }}</span></a>
                    </li>
                </ul>
            </div>
        </div>
