...
class PaymentGatewayRenderHelper
{
    public static function listOfPaymentGateways()
    {
        ...
        $payment_gateways=!empty($modules_payment_gateway) ? array_merge($payment_gateway_list, $modules_payment_gateway) : $payment_gateway_list;
    ...
    {
        $has_manual_payment_gateway = false;
        ...
        foreach ($all_gateway as $gateway) {
            ...
            if ($gateway['name'] == 'manual_payment')
            {
                ...
                $has_manual_payment_gateway = true;
            }
        ...
        if ($has_manual_payment_gateway) {
            $output .= '<div class="manual_payment_gateway_extra_field">
                            <div class="form-group single-input d-none manual_transaction_id mt-4">
                                ...
                                <input type="text" name="trasaction_id"     class="form-control form--control mt-2"  style="line-height: 2.5rem"  placeholder="'.__('Transaction ID').'">
                                <input type="file" name="manual_payment_image"  class="form-control form--control mt-2 icofont-line-height"  style="line-height: 2.5rem"  placeholder="'.__('Transaction Attachment').'" accept="image/*">
                           </div>
                         </div>';
        }
        // write code for all module extra info markup
