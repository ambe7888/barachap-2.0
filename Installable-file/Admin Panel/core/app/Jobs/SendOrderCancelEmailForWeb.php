...
class SendOrderCancelEmailForWeb implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $suborder=SubOrder::where('id',$this->sub_order_id)->first();
        
        // Get service ID for the given order
        $orders_services_id = SubOrder::where('id', $this->sub_order_id)
            ->pluck('service_id');

        // Get unique provider IDs
        $service_provider_id = Service::where('id', $orders_services_id)
        ...
        $mail_subject = get_static_option('order_cancel_email_subject') ?? __("Order Cancel #") . $suborder->service?->title;
        $message_for_provider_admin = get_static_option('order_cancel_admin_message') ?? __("One Order Cancel #") . $suborder->service?->title;
        $message_for_client = get_static_option('order_cancel_client_message') ?? __("You have successfully canceled an order #") . $suborder->service?->title;
