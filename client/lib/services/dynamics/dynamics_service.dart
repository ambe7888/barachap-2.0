...
import 'package:prohandy_client/services/dynamics/time_ago_helper.dart';
...
class DynamicsService with ChangeNotifier {
  ...
  getColors() async {
    ...
    if (lSlug == null) {
      ...
      debugPrint("Text direction set up done".toString());
    }
  ...
  setLangSlug(String slug, bool direction, {bool setLocally = false}) {
    if (languageSlug.length > 2) {
      final slugItems = slug.split("_"); // en_GB
      _appLocal = Locale(
        slugItems.firstOrNull ?? "",
        slugItems.lastOrNull ?? "",
      );
      debugPrint(slugItems.toString());
      try {
        timeago.setDefaultLocale(slugItems.firstOrNull ?? "en");
      } catch (e) {
...
class LanguageListModel {
  ...
  LanguageListModel({this.language});
