...
//    use EmailTemplateHelperTrait;
    ...
    public function userNewServiceApprovalTemplate(Request $request)
    {
        if($request->isMethod('post')){
            $request->validate([
        ...
        return view(self::BASE_PATH.'services.new-service-approve-template');
    }

    public function userNewServicePublishTemplate(Request $request)
    {
        if($request->isMethod('post')){
            $request->validate([
                'service_publish_subject'=>'required|min:5|max:100',
                'service_publish_message'=>'required|min:10|max:1000',
            ]);
            $fields = [
                'service_publish_subject',
                'service_publish_message',
            ];
            foreach ($fields as $field) {
                update_static_option($field, $request->$field);
            }
        ...
        return view(self::BASE_PATH.'services.new-service-publish-template');
    }

    public function userNewServiceUnpublishedTemplate(Request $request)
    {
        if($request->isMethod('post')){
            $request->validate([
                'service_unpublished_subject'=>'required|min:5|max:100',
                'service_unpublished_message'=>'required|min:10|max:1000',
            ]);
            $fields = [
                'service_unpublished_subject',
        ...
        return view(self::BASE_PATH.'services.new-service-unpublished-template');
    }
    ...
    public function newOrderTemplate(Request $request){
        if($request->isMethod('post')){

            $request->validate([
                'new_order_email_subject'=>'required|min:5|max:100',
                'new_order_client_message'=>'required|max:1000',
                'new_order_admin_provider_message'=>'required|max:1000',
                'new_order_admin_order_message'=>'required|max:1000',
            ]);
            $fields = [
                'new_order_email_subject',
                'new_order_client_message',
                'new_order_admin_provider_message',
                'new_order_admin_order_message',
            ];
            foreach ($fields as $field) {
                update_static_option($field, $request->$field);
            }
        ...
        return view('backend.pages.email-template.orders.new-order-template');
    }
