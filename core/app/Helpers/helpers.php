...
function get_whatsapp_option($key,$default = null)
{
    if(moduleExists('WhatsAppBookingSystem'))
    {
        ...
        $value = \Illuminate\Support\Facades\Cache::remember($option_name, 600, function () use($option_name) {
            try {
                return \Modules\WhatsAppBookingSystem\app\Models\WhatsAppDetails::where('option_name', $option_name)->first();
            }catch (\Exception $e){
                return null;
            }
        });
...
function update_whatsapp_option($key, $value)
{
   if(moduleExists('WhatsAppBookingSystem'))
   {
       if (!\Modules\WhatsAppBookingSystem\app\Models\WhatsAppDetails::where('option_name', $key)->first()) {
           \Modules\WhatsAppBookingSystem\app\Models\WhatsAppDetails::create([
               'option_name' => $key,
               'option_value' => $value
           ]);
       } else {
...
function single_post_share($url, $title, $img_url)
{
    ...
    $output .= '<li class="list-item"><a class="instagram-bg" href="' . $whatsapp_share_link . '"><i class="lab la-whatsapp"></i></a></li>';
    ...
    foreach ($menu_content as $menu_item) {
        ...
        if ($menu_item->ptype == 'static') {
            ...
            if ($menu_item->ptype == 'blog') {
                $menu_title = '';
                $menu_slug = '';
                $page_details = \Modules\Blog\app\Models\Blog::with('lang_front')->find($menu_item->pid);
                if (!empty($page_details)){
                    $menu_title = $page_details->title;
            ...
            $mega_menu_items = \Modules\Blog\app\Models\Blog::with('lang_front')->find($mega_menu_ids)->groupBy('category_id');
        }
...
{
    return \plugins\WidgetsBuilder\WidgetBuilderSetup::get_admin_panel_widgets();
}

function render_admin_saved_widgets($location)
{
    $output = '';
    $all_widgets = \App\Models\Backend\Widget::where(['widget_location' => $location])->orderBy('widget_order','asc')->get();
    foreach ($all_widgets as $widget) {
        $output .= \plugins\WidgetsBuilder\WidgetBuilderSetup::render_widgets_by_name_for_admin([
...
{
    return \plugins\WidgetsBuilder\WidgetBuilderSetup::get_admin_widget_sidebar_list();
}
...
function render_frontend_sidebar($location, $args = [])
{
    ...
    $all_widgets = \App\Models\Backend\Widget::where(['widget_location' => $location])->orderBy('widget_order', 'ASC')->get();
    foreach ($all_widgets as $widget) {
        $output .= \plugins\WidgetsBuilder\WidgetBuilderSetup::render_widgets_by_name_for_frontend([
            'name' => $widget->widget_name,
            'location' => $location,
            'id' => $widget->id,
...
function render_page_meta_data_for_category($category){
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('frontend.show.listing.by.category',$category->slug);
    $title = $category->name;
...
function render_page_meta_data_for_subcategory($subcategory){
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('frontend.show.listing.by.category',optional($subcategory)->slug) ?? '';
...
function render_page_meta_data_for_child_category($child_category){
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('frontend.show.listing.by.child.category',optional($child_category)->slug) ?? '';
...
function toastr_info($success){
    Toastr::info($success,__('Info!'), ["positionClass" => "toast-top-right","closeButton" => "true","progressBar" => "true"]);
}
...
function userListingLocation($listing){
    ...
    if (!empty(get_static_option('google_map_settings_on_off'))){
        ...
        if (is_null($listing_address)){
            $city_name = optional(optional($listing)->city)->city;
            $state_name = optional(optional($listing)->state)->state;
            $country_name = optional(optional($listing)->country)->country;
            // Add city name if available
            if (!empty($city_name)) {
                $listing_address .= $city_name;
    ...
    // if empty location return address
...
function userProfileLocation($user) {
    ...
    $user_state_name = optional(optional($user)->user_state)->state;
    $user_country_name = optional($user->user_country)->country;
    if (!empty($user_state_name) && !empty($user_country_name)) {
        $user_address = $user_state_name .', '. $user_country_name;
        return $user_address;
    }
    return $user->address;
}

function getCustomFont($status)
{
    return \App\Models\Backend\CustomFont::where('status', $status)->latest()->first();
}
...
function calculateMembershipRemainingTime($expireDate)
{
    if ($expireDate) {
        $expireDate = \Illuminate\Support\Carbon::parse($expireDate);
        $currentDate = \Illuminate\Support\Carbon::now();
        $diff = $currentDate->diff($expireDate);
        $output = '';

        if ($diff->y > 0) {
            $output .= $diff->y . ' ' . __('year') . ($diff->y > 1 ? 's ' : ' ');
        }

        if ($diff->m > 0) {
            $output .= $diff->m . ' ' . __('month') . ($diff->m > 1 ? 's ' : ' ');
        }

        if ($diff->d > 0) {
            $output .= $diff->d . ' ' . __('day') . ($diff->d > 1 ? 's ' : ' ');
        }

        return trim($output);
    }

    return ''; // Return empty string if $expireDate is null
}
...
function render_country_flag_markup_by_attachment_url($country_code)
{
    $flag_base_url = 'https://flagcdn.com/w320/';
    $flag_name_lower = strtolower($country_code);
    $flag_url = $flag_base_url . $flag_name_lower . '.png';

    $noImageMarkupRender =  asset('assets/uploads/no-image.png');
    $output = '';
    if(!empty($flag_url)){
        $output = '<img src="'.$flag_url.'" class="country-flag" alt="' . ($country_code ?? "") . '" height="100px" width="100px"/>';
    }else{
        $output = '<img src="'.$noImageMarkupRender.'" class="no-image" alt="' . ($country_code ?? "") . '" height="100px" width="100px"/>';
    }
    return $output;
}

function userProfileImageView($user_image_id){
    $output = '';
    if (!empty($user_image_id)){
        $output = render_image_markup_by_attachment_id($user_image_id);
    } else {
        $output = '<img src="' . asset('assets/frontend/img/static/user-no-image.webp') . '" alt="No Image" width="48px" height="48px">';
    }
    return $output;
}
...
function calculateOrderCouponAmount($couponCode, $total){
    $coupon_type = '';
    $coupon_amount = 0;
    ...
    if (!empty($couponCode)) {
        ...
        if ($coupon) {
            ...
            if ($coupon->expire_date > $current_date) {
                // Calculate the coupon amount based on the discount type
                if ($coupon->discount_type == 'percentage') {
                    $coupon_amount = ($total * $coupon->discount) / 100;
                    $total -= $coupon_amount;
                    $coupon_type = 'percentage';
                } else { // Assume 'amount' type
                    $coupon_amount = $coupon->discount;
                    $total -= $coupon_amount; // Apply discount to total
                    $coupon_type = 'amount';
                }
...
function subOrderCalculateCouponAmount($couponCode, $total){
    $coupon_type = '';
    $coupon_amount = 0;
...
if (!function_exists('generateInvoiceNumber')) {
    function generateInvoiceNumber()
    {
        ...
        $lastNumber = $lastOrder ? intval(substr($lastOrder->invoice_number, -4)) : 0;

        // Generate the new invoice number with a sequential suffix
        return $datePrefix . '-' . str_pad($lastNumber + 1, 4, '0', STR_PAD_LEFT);
    }
}

// create user notification
function admin_notification($last_order_id,$user_id, $type, $msg,$is_read){
    try {
        $validatedData = Validator::make(compact('last_order_id', 'user_id', 'type', 'msg', 'is_read'), [
            'last_order_id' => 'required|integer',
        ...
        AdminNotification::create([
            'identity' => $validatedData['last_order_id'],
...
// create user notification
function user_notification($last_order_id,$user_id, $type, $msg,$is_read){
    $notify = UserNotification::create([
        'identity'=> $last_order_id,
        'user_id'=> $user_id,
        'type'=>$type,
        'message'=>$msg,
        'is_read'=>$is_read
...
function calculateTaxBasedOnCoordinates($id, $type)
{
    ...
    // Determine the location based on the type
    if ($type == 'order') {
        $location = \App\Models\SubOrderLocation::where('id', $id)->first();
    } elseif ($type == 'job') {
        $location = \Modules\JobPost\app\Models\JobPostLocation::where('id', $id)->first();
    } else {
    ...
    // Handle tax-inclusive or tax-exclusive settings
    if (!empty(get_static_option('tax_inclusive_exclusive')) && get_static_option('tax_inclusive_exclusive') == 'inclusive') {
        $state_tax_rate = null;
    } elseif ($location !== null) {
        // Get state and city tax rates
        $state_id = $location->state_id;
        $city_id = $location->city_id;
        $state_tax = StateTax::where('state_id', $state_id)->first();

        $city_tax = CityTax::where('city_id', $city_id)->first();

        if (!empty($city_tax) && $city_tax->tax_rate >= 0) {
            $state_tax_rate = $city_tax->tax_rate;
        } else {
            $state_tax_rate =  $state_tax?->tax_rate ;
        }

        // If tax rate is still 0, calculate based on latitude/longitude
        if ($state_tax_rate == null && $state_tax_rate == '') {
                ...
                if (isset($geocode_data['status']) && $geocode_data['status'] == 'OK') {
                    ...
                    if ($state && $city) {
                        ...
                        if ($get_state !== null && $get_city !== null) {
                            $state_tax = StateTax::where('state_id', $get_state->id)->first();
                            $city_tax = CityTax::where('city_id', $get_city->id)->first();
        ...
        if (empty($state_tax_rate) || !is_numeric($state_tax_rate) || $state_tax_rate < 0) {
    ...
    // Ensure $state_tax_rate is a valid numeric value
    if (empty($state_tax_rate) || !is_numeric($state_tax_rate) || $state_tax_rate < 0) {
        $state_tax_rate = get_static_option('tax_rate_by_country') ?? 0;
    }
