...
class SupportTicketController extends Controller
{
    ...
    public function search_ticket(Request $request)
    {
        ...
        $tickets = Ticket::where(function ($query) use ($searchTerm, $priority, $status) {
            if ($searchTerm) {
    ...
    public function ticket_details(Request $request, $id,$notificationId=null){
        ...
        if($request->isMethod('post')){
            ...
            // send notification to user
             user_notification($ticket_details->id,$ticket_details->user_id,'ticket',__('Ticket updates available'), 'unread');
            ...
            if($request->email_notify == 'on'){
                ...
                try {
                    ...
                    $message = str_replace(["@name","@ticket_id"],[$ticket_details->user?->fullname ,$id], $message);
                    $message .= "\n\nMessage:\n" . ($request->message ?? '');
                    $message = nl2br($message); // Convert \n to <br>
                    // Add image download link
                    if (!empty($imageName)) {
                        $attachmentUrl = asset('assets/uploads/ticket/chat-messages/' . $imageName);

                        $message .= "\n\nAttachment:\n <a href=\"{$attachmentUrl}\" download class=\"supportTicket_single__uploads\">
                            <i class=\"fa-solid fa-cloud-arrow-up\"></i> Download Attachment
                        </a>";
                        $message = nl2br($message); // Convert \n to <br>

                    }

                    Mail::to($ticket_details->user?->email)->send(new BasicMail([
