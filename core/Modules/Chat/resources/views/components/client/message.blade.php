@php
    $project = json_decode(json_encode($message->message['service']));
@endphp
        ...
        <div class="chat-wrapper-details-inner-chat-flex">
            <div class="chat-wrapper-details-inner-chat-thumb">
                @if($data->client?->image)
                    <img src="{{ asset('assets/uploads/author.jpg') }}" alt="">
                @else
            ...
            <div class="chat-wrapper-details-inner-chat-contents {{ !empty($project->type) ? "bg-danger p-2 text-dark bg-opacity-10" : "" }}">
                <p class="chat-wrapper-details-inner-chat-contents-para {{ !empty($project) ? "d-none" : "" }}">
                    ...
                    @if(!empty($message->file))
                        ...
                        <a  style="height:400px;width:400px;" href="{{  $message->file }}" ><img src="{{ $message->file }}" alt=""></a>
                        <?php
                            $ext = pathinfo($message->file, PATHINFO_EXTENSION);
                        ?>
                            @if($ext == 'pdf')
                            <a class="download-pdf-chat" href="{{  $message->file }}" download>{{ __('Download pdf') }}</a>
                        @endif
                    @endif
                </p>
                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-md-8">
                                <div class="card-body">
                                    ...
                                    @if(($project->type ?? '') == 'job')
                                        <a class="btn btn-primary btn-sm" target="_blank" href="{{ route('api.job.post.details',  $project->slug) }}">{{ __('View details') }}</a>
                                    @else
                                        <a class="btn btn-primary btn-sm" target="_blank" href="{{ route('api.service.details', $project->slug) }}">{{ __('View details') }}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
...
@if($message->from_user == 2)
    <div class="chat-wrapper-details-inner-chat">
        <div class="chat-wrapper-details-inner-chat-flex">
            <div class="chat-wrapper-details-inner-chat-thumb">
                 <a href="{{ route('api.provider.profile.details', $data?->provider?->username) }}" target="_blank">
                    @if($data->provider?->image)
                        <img src="{{ asset('assets/uploads/profile/'.$data->provider?->image) }}" alt="">
                    @else
                        <img src="{{ asset('assets/static/img/author/author.jpg') }}" alt="{{ __('author') }}">
                    @endif
                </a>
