...
<script>
    ...
    */
    ...
    $(document).on("click","#provider-send-message-to-client", function (){
        ...
        if(messages_ != '' || file !== undefined){
            ...
            send_ajax_request("post", form, "{{ route("provider.message.send") }}", function (){}, function (response){
    ...
    function fetch_chat_data(client_id, page = 1, callback){
        ...
        formData.append("provider_id","{{auth('web')->id()}}")
        formData.append("_token", "{{ csrf_token() }}");
        formData.append("from_user", 2)

        send_ajax_request("post", formData,`{{ route("provider.fetch.chat.client.record") }}?page=${page}`,function (){
