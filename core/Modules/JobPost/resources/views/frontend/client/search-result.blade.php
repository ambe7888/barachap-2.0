    ...
    @foreach($all_jobs as $job)
        <tr>
            ...
            <td>
                ...
                @endif
                <span class="my-2"><x-status.client-job-published-change :url="route('client.job.published.status.change',$job->id)"/></span>
            </td>

            <td class="actions">
                <a class="cmnBtn btn_5 btn_bg_info radius-5" href="{{ route('client.offer-job.all', $job->id) }}">
                    {{ __('Job Offers') }}
                </a>
                <x-icon.view-icon :url="route('client.jobs.details',$job->id)"/> 
                <x-icon.edit-icon :url="route('client.jobs.edit',$job->id)"/>
                <x-popup.delete-popup :url="route('client.jobs.delete',$job->id)"/>
            </td>
        </tr>
    @endforeach
    </tbody>
