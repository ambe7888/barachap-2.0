...
class PluginManageController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:plugins-add', ['only' => ['index', 'add_new', 'store_plugin', 'delete_plugin', 'change_status']]);
    }
    ...
    public function store_plugin(Request $request)
    {
        ...
        // work for upload plugin
        //validate the plugin file is valid or not.. it has contain  meta data or not
        ...
        // if plugin file uploaded show option to active that plugin.. or redirect theme to all plugins page
    ...
    private function replacePluginFile($file)
    {
        ...
        if ($zipExtracted) {
            ...
            // move full folder into module_path folder
            ...
            if ($plugin_info->alias === 'whatsappbookingsystem') {
    ...
    public function move_whatsapp_booking_token_generate(): void
    {
    ...
    public function delete_plugin(Request $request)
    {
        ...
        // remove this name from modules_status.json file
