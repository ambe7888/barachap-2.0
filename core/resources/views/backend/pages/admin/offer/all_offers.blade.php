            ...
            <div class="dashboard__card bg__white padding-20 radius-10 mb-2">
                ...
                <div class="dashboard__inner__header">
                    <div class="dashboard__inner__header__flex">
                        ...
                        <div class="dashboard__inner__header__right">
                            @can('offer-add')
                                <a href="{{ route('admin.offer.add') }}" class="cmnBtn btn_5 btn_bg_blue radius-5">{{ __('Add Offer') }}</a>
                            @endcan
                ...
                <div class="tableStyle_three mt-4">
                    <div class="table_wrapper custom_Table">
                        <div class="search_offer_result">
                            @include('backend.pages.admin.offer.search-offer')
                        </div>
                    </div>
                </div>
            </div>
...
@section('scripts')
    @can('admin-offer-bulk-delete')
        <x-bulk-action.bulk-action-js :url="route('admin.offer.bulk.action')"/>
    @endcan
        ...
        (function(){
            ...
            $(document).ready(function(){
                    ...
                    $.ajax({
                        ...
                        success:function(res){
                            if(res.status=='nothing'){
                                $('.search_offer_result').html('<h3 class="text-center text-danger">'+"{{ __('Nothing Found') }}"+'</h3>');
                            }else{
                                $('.search_offer_result').html(res);
                            }
                        }
                    });
                });
                ...
                function notices(page,string_search){
                    $.ajax({
                        url:"{{ route('admin.offer.paginate').'?page='}}" + page,
                        data:{string_search:string_search},
                        success:function(res){
                            $('.search_offer_result').html(res);
                        }
