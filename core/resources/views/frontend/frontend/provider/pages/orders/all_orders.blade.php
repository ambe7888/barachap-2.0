...
@section('content')
        ...
        <div class="col-xl-12 col-lg-12">
            <div class="dashboard__card bg__white padding-20 radius-10">
                ...
                <div class="tableStyle_three mt-4">
                    @include('frontend.frontend.provider.pages.orders.order-filter')
                    <div class="table_wrapper custom_Table">
                        <div class="search_result">
                            @include('frontend.frontend.provider.pages.orders.search-order')
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal fade" id="OrderStatusChangeModal" tabindex="-1" role="dialog"
         ...
         aria-hidden="true">
        <form action="{{ route('provider.order.status.change') }}" method="post">
            @csrf
    ...
    @include('frontend.frontend.provider.pages.orders.manual-payment-file-modal')
@endsection
@section('scripts')
        ...
        (function(){
            ...
            $(document).ready(function(){
                ...
                $(document).on('keyup','.string_search',function(){
                    ...
                    $.ajax({
                        url:"{{ route('provider.order.search') }}",
                        method:'GET',
                ...
                function admin_orders(page, status){
                    $.ajax({
                        url:"{{ route('provider.order.paginate').'?page='}}" + page + "&status=" + status,
                        success:function(res){
                            $('.search_result').html(res);
                        }
                    });
