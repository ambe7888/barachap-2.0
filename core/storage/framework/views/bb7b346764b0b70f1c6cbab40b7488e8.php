...
<?php
    $type=auth()->check() ? auth()->user()->type : 'admin';
  if($type === 1)
    {
        $type = 'client';
    }else if($type === 0)
    ...
    (function ($) {
        ...
        Dropzone.options.placeholderfForm = {
            ...
            success: function (file, response) {
                var isDemoMiddlewareEnabled = <?php echo isset($isDemoMiddlewareIsEnabled) && $isDemoMiddlewareIsEnabled ? 'true' : 'false'; ?>;
                if (file.previewElement) {
                   if(isDemoMiddlewareEnabled)
                   {
                    file.previewElement.classList.add("dz-error");
                    for (let node of file.previewElement.querySelectorAll("[data-dz-errormessage]")) {
                        node.textContent = "This is demonstration purpose only, you may not able to change few settings, once your purchase this script you will get access to all settings.";
                    }
                   }
                   else
                   {
...
<?php /**PATH C:\xampp\htdocs\barachap\web\core\resources\views/components/media/js.blade.php ENDPATH**/ ?>